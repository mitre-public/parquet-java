...
public class SchemaControlEncryptionTest {
  ...
  // In the test We use a map to tell WriteSupport which columns to be encrypted with what key. In real use cases,
  // people
  ...
  private class CryptoGroupWriteSupport extends GroupWriteSupport {
    ...
    private void setMetadata(ColumnDescriptor column, Configuration conf) {
      ...
      if (cryptoMetadata.containsKey(columnShortName)
        ...
        conf.set(
            SchemaCryptoPropertiesFactory.PATH_NAME_PREFIX + columnKey,
            cryptoMetadata
                .get(columnShortName)
                .get("columnKeyMetaData")
                .toString());
      }
    }
  }
